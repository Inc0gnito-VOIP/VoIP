    ...
    with open(CSV_PATH, encoding='utf-8') as f:
        for row in csv.DictReader(f):
            sessions.append({
                ...
                'label': int(row['label']),
                'risk_score': float(row['risk_score']) if 'risk_score' in row else float(row['label']),
            })
    return sessions
...
def api_stats():
    ...
    if not sessions:
        return jsonify({
            'total': 0, 'normal_count': 0, 'fraud_count': 0,
...
<style>
  ...
  *{margin:0;padding:0;box-sizing:border-box;}
  ...
  .stat-card{background:var(--color-card);border:1px solid var(--color-border);border-radius:12px;padding:20px 24px;position:relative;overflow:hidden;}
  ...
  .stat-label{font-size:0.7rem;color:var(--color-muted);text-transform:uppercase;letter-spacing:0.1em;margin-bottom:8px;font-family:'JetBrains Mono',monospace;}
  ...
  .stat-sub{font-size:0.75rem;color:var(--color-muted);margin-top:6px;font-family:'JetBrains Mono',monospace;}
  ...
  .chart-title{font-size:0.8rem;color:var(--color-muted);text-transform:uppercase;letter-spacing:0.08em;margin-bottom:20px;font-family:'JetBrains Mono',monospace;}
  ...
  .filter-btn{padding:6px 14px;border-radius:6px;border:1px solid var(--color-border);background:transparent;color:var(--color-muted);cursor:pointer;font-family:'JetBrains Mono',monospace;font-size:0.72rem;transition:all 0.2s;}
  .filter-btn.active,.filter-btn:hover{border-color:var(--accent);color:var(--accent);}
  table{width:100%;border-collapse:collapse;}
  th{text-align:left;padding:10px 14px;font-size:0.68rem;color:var(--color-muted);text-transform:uppercase;letter-spacing:0.08em;border-bottom:1px solid var(--color-border);font-family:'JetBrains Mono',monospace;}
  td{padding:12px 14px;font-size:0.82rem;font-family:'JetBrains Mono',monospace;border-bottom:1px solid rgba(51,65,85,0.5);}
  ...
  .badge-risk-high{background:rgba(239,68,68,0.12);color:var(--color-fraud);border:1px solid rgba(239,68,68,0.3);margin-left:6px;}
  .badge-risk-medium{background:rgba(245,158,11,0.12);color:var(--color-warning);border:1px solid rgba(245,158,11,0.3);margin-left:6px;}
  .badge-risk-low{background:rgba(34,197,94,0.12);color:var(--color-normal);border:1px solid rgba(34,197,94,0.3);margin-left:6px;}
  .jitter-bar{display:flex;align-items:center;gap:8px;}
  .jitter-track{flex:1;height:4px;background:var(--color-border);border-radius:2px;overflow:hidden;}
...
<main>
  ...
  <div class="table-card">
    ...
    <div style="overflow-x:auto;max-height:400px;overflow-y:auto;">
      <table>
        <thead><tr><th>Session ID</th><th>Label</th><th>Risk</th><th>Avg Jitter</th><th>Avg Latency</th><th>Packet Loss</th><th>Seq Gap Rate</th><th>Packets</th></tr></thead>
...
function riskBadge(score){
  if(score>0.7) return '<span class="badge badge-risk-high">HIGH</span>';
  if(score>0.4) return '<span class="badge badge-risk-medium">MED</span>';
...
function renderTable(sessions){
  ...
  document.getElementById('session-table').innerHTML=sessions.slice(0,50).map(s=>{
    ...
    const color=s.label===1?'#ef4444':'#22c55e';
